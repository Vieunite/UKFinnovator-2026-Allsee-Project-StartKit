...
export function Legend({
  ...
  return (
    <Headless.Legend
      ...
      className={clsx(
        className,
        'dark:text-textDarkMode text-base/6 font-semibold text-zinc-950 data-[disabled]:opacity-50 sm:text-sm/6'
      )}
    />
  )
}
...
export function Label({ className, ...props }: { className?: string } & Omit<Headless.LabelProps, 'as' | 'className'>) {
  return (
    <Headless.Label
      ...
      className={clsx(
        className,
        'dark:text-textDarkMode select-none text-base/6 text-zinc-950 data-[disabled]:opacity-50 sm:text-sm/6'
      )}
    />
  )
}
